  ...
  function Calenda(_a) {
    ...
    this.moveMonth = function (direction) {
      ...
      return direction === "forward"
        ? (function () {
            var prevMonth = (_this.prevMonth = _this.currentMonth),
              currentMonth = (_this.currentMonth =
                (_this.currentMonth + 1) % 12);
            _this.startingDay =
              (currentMonthLength -
                (7 * Math.floor(currentMonthLength / 7) -
                  _this.startingDay +
                  1) +
                1) %
        ...
        : direction === "backward" &&
            (function () {
              var monthEndDayNum = _this.startingDay - 1,
                finalMonthEndDayNum = monthEndDayNum < 0 ? 6 : monthEndDayNum,
                newMonthIdx = _this.monthIdx - 1,
                getMonth = newMonthIdx < 0 ? 11 : newMonthIdx,
                monthEndDay = getMonthLength(getMonth),
                prevMonth = (_this.prevMonth = _this.currentMonth),
                currentMonth = (_this.currentMonth = getMonth);
              _this.startingDay = getStartingDay(
                finalMonthEndDayNum,
                monthEndDay
              );
              ((prevMonth === 11 && currentMonth === 0) ||
